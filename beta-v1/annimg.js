  ...
  prepImage: function(img) {
    ...
    for (var j=areas.length-1;j>=0;j--) {
      if (areas[j].getAttribute('shape').toLowerCase() == 'rect') {
        ...
        a.className = 'annotation weird';
        a.id="picture"+(j+1).toString();
    ...
    // for(var j=areas.length;j>=1;j--){
    //   var s=j+1;
    //   var f=document.getElementById("picture"+(s).toString());
    //   f.onclick=function(){project(s);};
    // }
  },
